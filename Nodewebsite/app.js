...
/* groups page
*
* Searches through db to find grouped genome samples based on email

app.get('/group_samples', function (req, res) {
    if (req.session.userStatus == "loggedIn") {
        userLoggedIn = true;
    } else {
        userLoggedIn = false;
        ...
        client.query("SELECT * FROM group_samples WHERE email='" + email + "'", (err, groupSamp_results) => {
            ...
            if (groupSamp_results.rows.length > 0) {
                ...
                client.query(selectSQL + ");", (err, group_samples) => {
                    ...
                    console.log(haveGroupSamples);
                    res.render('pages/groups', { userLoggedIn: userLoggedIn, group_samples: group_samples.rows, haveGroupSamples: haveGroupSamples });
                });
            }
            else {
                res.render('pages/groups', { userLoggedIn: userLoggedIn, group_samples: 0, haveGroupSamples: false });
            }
        });
    }
}
